...
`::tk::ConsoleOutput` with custom tags, or adding extra menu items and
buttons to the console window itself (see
[the Tcler's Wiki](https://wiki.tcl-lang.org/page/console) for examples in
active use for close to 20 years).

None of this is documented as a stable interface today, and the console
manual page itself says as much. As part of making `console` an officially
supported command on Unix, it would be worth the core team also considering
`console eval`/`consoleinterp` and the small set of internals they expose
(`::tk::ConsoleOutput` in particular) as a semi-public interface worth
